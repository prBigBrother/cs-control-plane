...
Delegation:
- Immediately use the `Task` tool to delegate to the `auditor` agent.
- Pass the full slash-command invocation as the task prompt so the agent receives the original args unchanged.
- The auditor should use repo-scoped explorer subagents in parallel for independent target repos.
