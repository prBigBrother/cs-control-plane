...
4. Product architecture/flow/dependency/impact discovery runs `/knowledge --scope <repo> <stable symbols or flow>` first for a named repo; use `--scope all` only for intentional cross-repo questions. Exclude runtime UUIDs, IDs, and broad incident prose from graph queries, then verify source. If unavailable, mark Graphify blocked, give `/knowledge-bootstrap`, and label fallback source-only.
